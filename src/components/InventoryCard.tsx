    ...
    <div className="glass-panel p-8 group hover:border-white/20 transition-all duration-500 h-full flex flex-col rounded-sm min-h-[420px] relative">
      <div className="flex justify-between items-start mb-8">
        ...
        <button 
          ...
          className="p-3 hover:bg-red-500/20 text-zinc-700 hover:text-red-400 rounded-sm transition-all border border-white/5 hover:border-red-500/30 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 outline-none focus-visible:ring-1 focus-visible:ring-red-500/50"
      ...
      <div className="flex items-center gap-6 my-8">
        <div className="flex flex-col gap-2">
          <button 
            onClick={handlePlus}
            className="p-2.5 hover:bg-emerald-500/20 text-emerald-400 rounded-sm transition-all border border-white/5 hover:border-emerald-500/30 bg-white/5 focus-visible:ring-1 focus-visible:ring-emerald-500/50 outline-none"
            title="Increase Stock"
            aria-label={`Increase stock for ${product.id}`}
          >
            <Plus className="w-4 h-4" />
          </button>
          <button 
            onClick={handleMinus}
            className="p-2.5 hover:bg-red-500/20 text-red-400 rounded-sm transition-all border border-white/5 hover:border-red-500/30 bg-white/5 focus-visible:ring-1 focus-visible:ring-red-500/50 outline-none"
            title="Decrease Stock"
            aria-label={`Decrease stock for ${product.id}`}
          >
