    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === '/' && document.activeElement?.tagName !== 'INPUT' && document.activeElement?.tagName !== 'TEXTAREA') {
        ...
        if (activeTab !== 'dashboard') {
          ...
          // Wait for tab transition and element to be in DOM
          ...
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="glass-panel p-12 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-500">
                ...
                <p className="text-zinc-500 text-sm max-w-xs mb-8">We couldn't find any products matching "{searchQuery}". Try a different search term or clear the filter.</p>
                <button
                  onClick={() => {
                    setSearchQuery('');
                    searchInputRef.current?.focus();
                  }}
                  className="px-8 py-3 glass-panel text-white text-[10px] font-black uppercase tracking-widest hover:bg-white/5 transition-all active:scale-95"
                >
                  Clear Search
                </button>
              </div>
