  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === '/' &&
        ...
        // Small delay to ensure tab is rendered before focusing
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="glass-panel p-20 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-500">
                <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mb-6 border border-white/10">
                  <PackageSearch className="w-10 h-10 text-zinc-500" />
                </div>
                ...
                <p className="text-zinc-500 max-w-xs mb-8">
                  We couldn't find any products matching <span className="text-sky-400 font-mono">"{searchQuery}"</span>.
                </p>
                <button
                  onClick={() => {
                    setSearchQuery('');
                  ...
                  className="px-8 py-3 glass-card text-sky-400 text-[10px] font-black uppercase tracking-widest hover:bg-sky-500/10 hover:border-sky-500/40 transition-all active:scale-95"
                >
                  Clear Search
                </button>
              </div>
              ...
              <div className="glass-panel p-20 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-zinc-500 max-w-xs mb-8">
                  Your industrial warehouse is currently empty. Start by adding your first product.
                </p>
                <button
                  onClick={() => setIsNewProductModalOpen(true)}
                  className="px-8 py-3 glass-card text-sky-400 text-[10px] font-black uppercase tracking-widest hover:bg-sky-500/10 hover:border-sky-500/40 transition-all active:scale-95"
