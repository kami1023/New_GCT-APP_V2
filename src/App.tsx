...
import { LayoutDashboard, ReceiptText, History, PlusCircle, Search, Settings as SettingsIcon, PackageSearch, AlertTriangle, Sparkles, X, PackageOpen } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === '/' && activeTab === 'dashboard' && document.activeElement?.tagName !== 'INPUT' && document.activeElement?.tagName !== 'TEXTAREA') {
        e.preventDefault();
          ...
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="relative w-full md:w-96">
                ...
                {searchQuery && (
                  <button
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 p-1 hover:bg-white/10 rounded-full transition-colors group"
                    aria-label="Clear search"
                  >
                    <X className="w-3 h-3 text-zinc-500 group-hover:text-white" />
                  </button>
                )}
              </div>
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 animate-in fade-in zoom-in-95 duration-500">
                ...
                <p className="text-zinc-500 text-sm mb-8">No results match "{searchQuery}"</p>
                <button
                  onClick={() => setSearchQuery('')}
                  className="px-6 py-3 glass-panel text-sky-400 text-[10px] font-black uppercase tracking-widest hover:bg-sky-500/10 transition-all active:scale-95"
                >
                  Clear Search
                </button>
              </div>
