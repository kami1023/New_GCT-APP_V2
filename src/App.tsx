        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="relative w-full md:w-96 group">
                ...
                {searchQuery && (
                  <button
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 p-1 hover:bg-white/10 rounded-full transition-colors text-zinc-400 hover:text-white"
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-32 glass-panel border-white/5 animate-in fade-in zoom-in duration-500" aria-live="polite">
                <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-6 border border-white/10">
                  <PackageSearch className="w-8 h-8 text-zinc-600" />
                </div>
                <h3 className="text-xl font-bold text-white mb-2">No products found</h3>
                <p className="text-zinc-500 mb-8 max-w-xs text-center text-sm">We couldn't find any products matching "{searchQuery}"</p>
                <button
                  onClick={() => {
                    setSearchQuery('');
                    searchInputRef.current?.focus();
                  }}
                  className="px-6 py-3 glass-card text-sky-400 text-[10px] font-black uppercase tracking-widest hover:bg-sky-500/10 transition-all active:scale-95"
                >
                  Clear Search
                </button>
              </div>
