  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === '/' && document.activeElement?.tagName !== 'INPUT' && document.activeElement?.tagName !== 'TEXTAREA') {
        ...
        // Small delay to ensure the tab has switched and the input is rendered
        setTimeout(() => {
          ...
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {(() => {
              ...
              if (filteredProducts.length === 0) {
                return (
                  <div className="flex flex-col items-center justify-center py-24 glass-panel border-white/5 animate-in fade-in duration-500">
                    ...
                    <p className="text-zinc-500 text-sm mb-8">Try adjusting your search terms or filters</p>
                    <button
                      onClick={() => {
                        setSearchQuery('');
                        searchInputRef.current?.focus();
                      }}
                      className="px-6 py-3 glass-panel text-sky-400 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-sky-500/10 transition-all active:scale-95"
                    >
