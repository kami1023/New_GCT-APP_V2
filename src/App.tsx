          ...
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="relative w-full md:w-96 group">
                ...
                {searchQuery && (
                  <button
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 p-1 hover:bg-white/10 rounded-full transition-all text-zinc-500 hover:text-white"
            ...
            {filteredProducts.length === 0 && searchQuery ? (
              <div className="flex flex-col items-center justify-center py-20 glass-panel border-dashed border-white/10">
                <PackageSearch className="w-16 h-16 text-zinc-700 mb-4" />
                <h3 className="text-xl font-bold text-white mb-2">No products found</h3>
                <button onClick={() => { setSearchQuery(''); searchInputRef.current?.focus(); }} className="px-6 py-2 glass-card text-sky-400 text-[10px] font-black uppercase tracking-widest hover:bg-sky-500/10 transition-all">Clear Search</button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
                {filteredProducts.map(product => (
                  <div key={product.id} className="inventory-card-container h-full flex flex-col gap-4">
                    <InventoryCard product={product} onUpdateStock={handleUpdateStock} onUpdatePrice={handleUpdatePrice} onDelete={handleDeleteProduct} />
                  </div>
                ))}
              </div>
            )}
